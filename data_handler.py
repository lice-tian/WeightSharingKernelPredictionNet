...
import os
import numpy as np
from image_utils import load_exr, load_exr_one_channel, bmfr_gamma_correction
...
class DataHandler:
    ...
    def load_dataset(self):
        if os.path.exists(self.dataset_name):
            ...
            self.dataset = torch.load(self.dataset_name)
        ...
        if self.subset == 'train' or self.subset == 'valid':
            ...
            for scene_name in self.scene_list:
                ...
                for idx in range(60):
                    ...
                    #scatter2_depth_path = os.path.join(data_subdir, 'scattar2_depth' + exr_name)
                    scattar1_color_path = os.path.join(data_subdir, 'scattar1_color_' + exr_name)
                    scattar2_color_path = os.path.join(data_subdir, 'scattar2_color_' + exr_name)
                    scatter1_transmission_path = os.path.join(data_subdir, 'transmission1_' + exr_name)
                    scatter1_transmission_path = os.path.join(data_subdir, 'transmission2_' + exr_name)
                    #scatter1_scatter_rate_path = os.path.join(data_subdir, 'scatter_rate1_' + exr_name)
                    #scatter2_scatter_rate_path = os.path.join(data_subdir, 'scatter_rate2_' + exr_name)
                    ...
                    scattar1_color = bmfr_gamma_correction(load_exr(scattar1_color_path, datatype=np.float32, axis=0))
                    scattar2_color = bmfr_gamma_correction(load_exr(scattar2_color_path, datatype=np.float32, axis=0))
                    scatter1_transmission = load_exr(scatter1_transmission_path, datatype=np.float32, axis=0)
                    scatter2_transmission = load_exr(scatter2_transmission_path, datatype=np.float32, axis=0)

                    # ? original noisy ranges between [0, infinity] ==> [0,1]
                    noisy = bmfr_gamma_correction(load_exr(noisy_path, datatype=np.float32, axis=0))
                    # ? original GT ranges between [0, infinity] ==> [0,1]
                    real = bmfr_gamma_correction(load_exr(real_path, datatype=np.float32, axis=0))

                    noisy_full_img = np.concatenate((noisy, albedo, normal, depth, scattar1_color, scattar2_color, scatter1_transmission, scatter2_transmission), axis=0)
            ...
            torch.save(self.dataset, self.dataset_name)
            ...
            for scene_name in self.scene_list:
                ...
                for idx in range(60):
                    ...
                    # ? original noisy ranges between [0, infinity] ==> [0,1]
                    noisy = bmfr_gamma_correction(load_exr(noisy_path, datatype=np.float16, axis=0))
                    # ? original GT ranges between [0, infinity] ==> [0,1]
                    real = bmfr_gamma_correction(load_exr(real_path, datatype=np.float32, axis=0))
