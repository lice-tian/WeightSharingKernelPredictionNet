...
def bmfr_gamma_correction(color):
    gamma = 0.45
    correction_color = color / np.max(color)
    correction_color = np.power(correction_color, gamma)
...
def bmfr_gamma_correction_torch(color) -> torch.Tensor:
    gamma = 0.45
